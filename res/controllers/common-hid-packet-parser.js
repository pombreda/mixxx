...
// Return bitvector bit offset based on bitmask
...
HIDModifierList.prototype.set = function(name, value) {
    if ( (!name in this.modifiers) ) {
        HIDDebug("Trying to set unknown modifier: " + name);
        return;
    }
...
HIDModifierList.prototype.get = function(name) {
    if ( (!name in this.modifiers) ) {
        HIDDebug("Trying to get unknown modifier: " + name);
        return false;
    }
...
HIDModifierList.prototype.setCallback = function(name, callback) {
    if ( (!name in this.modifiers) ) {
        HIDDebug("Trying to set callback for unknown modifier: " + name);
        return;
    }
...
HIDPacket.prototype.setMinDelta = function(group, name, mindelta) {
    ...
    if (field == undefined) {
        HIDDebug("No such field when adjusting mindelta: " + group +" " + name);
        return;
    }
...
HIDController.prototype.setCallback = function(packet, group, name, callback) {
    ...
    if (input_packet == undefined) {
        HIDDebug("setCallback: input packet not found: " + packet);
        return;
    }
    ...
    for (var packet_name in this.OutputPackets) {
        ...
        for (var group_name in packet.groups) {
            ...
            for (var field_name in group) {
                ...
                if (value) {
                    this.setOutput(field.group,field.name, this.LEDColors[this.deckOutputColors[deck]])
                } else {
                    this.setOutput(field.group,field.name, this.LEDColors.off)
                }

...
// Copyright (C) 2012-2014, Ilkka Tuohela
...
function KontrolF1Controller() {
    ...
    this.controlModeButtons = { "decks": "capture",  "samplers": "quant" };
    this.defaultControlMode = "decks";

    this.scalers = new Object();
    this.knobs = new Object();
    this.faders = new Object();
    this.grids = new Object();
    this.playbuttons = new Object();
    this.segments = new Object();

    this.segments['empty'] = [0, 0, 0, 0, 0, 0, 0];
    this.segments[0] = [0x00, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40];
    this.segments[1] = [0x00, 0x40, 0x40, 0x00, 0x00, 0x00, 0x00];
    this.segments[2] = [0x40, 0x0,  0x40, 0x40, 0x00, 0x40, 0x40];
    this.segments[3] = [0x40, 0x40, 0x40, 0x40, 0x00, 0x00, 0x40];
    this.segments[4] = [0x40, 0x40, 0x40, 0x00, 0x40, 0x00, 0x00];
    this.segments[5] = [0x40, 0x40, 0x00, 0x40, 0x40, 0x00, 0x40];
    this.segments[6] = [0x40, 0x40, 0x00, 0x40, 0x40, 0x40, 0x40];
    this.segments[7] = [0x00, 0x40, 0x40, 0x40, 0x00, 0x00, 0x00];
    this.segments[8] = [0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40];
    this.segments[9] = [0x40, 0x40, 0x40, 0x40, 0x40, 0x00, 0x40];
...
KontrolF1.shutdown = function() {
    ...
    KontrolF1.resetLEDs();
    HIDDebug("NI Traktor F1 " + KontrolF1.id + " shutdown");
}
...
KontrolF1.updateLEDs = function() {
    var packet = KontrolF1.controller.getOutputPacket("lights");
    packet.send();
}
...
KontrolF1.registerInputPackets = function() {
    var controller = KontrolF1.controller;
    ...
    KontrolF1.controller.registerInputPacket(packet);

}

KontrolF1.registerOutputPackets = function() {
    var controller = KontrolF1.controller;
    var packet = undefined;

    packet = new HIDPacket("lights", [0x80], 81);

    // Right 7-segment number - 0x0 off, 0x40 on
    ...
    // Left 7-segment number - 0x0 off, 0x40 on
    ...
    KontrolF1.controller.registerOutputPacket(packet);
...
KontrolF1.registerCallbacks = function() {
    ...
    controller.setCallback("control", "hid", "knob_1",  KontrolF1.knobCallback);
    controller.setCallback("control", "hid", "knob_2",  KontrolF1.knobCallback);
    controller.setCallback("control", "hid", "knob_3",  KontrolF1.knobCallback);
    controller.setCallback("control", "hid", "knob_4",  KontrolF1.knobCallback);

    controller.setCallback("control", "hid", "fader_1", KontrolF1.faderCallback);
    controller.setCallback("control", "hid", "fader_2", KontrolF1.faderCallback);
    controller.setCallback("control", "hid", "fader_3", KontrolF1.faderCallback);
    controller.setCallback("control", "hid", "fader_4", KontrolF1.faderCallback);

    controller.setCallback("control", "hid", "grid_1",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_2",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_3",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_4",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_5",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_6",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_7",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_8",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_9",  KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_10", KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_11", KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_12", KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_13", KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_14", KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_15", KontrolF1.gridCallback);
    controller.setCallback("control", "hid", "grid_16", KontrolF1.gridCallback);

    controller.setCallback("control", "hid", "play_1", KontrolF1.playCallback);
    controller.setCallback("control", "hid", "play_2", KontrolF1.playCallback);
    controller.setCallback("control", "hid", "play_3", KontrolF1.playCallback);
    controller.setCallback("control", "hid", "play_4", KontrolF1.playCallback);
    ...
    KontrolF1.linkGrid("decks", "grid_1",  "[Channel1]", "hotcue_1",    false, KontrolF1.hotcueCallback, [0x7f, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_2",  "[Channel1]", "hotcue_2",    false, KontrolF1.hotcueCallback, [0x7f, 0x00, 0x7f]);
    KontrolF1.linkGrid("decks", "grid_3",  "[Channel2]", "hotcue_1",    false, KontrolF1.hotcueCallback, [0x7f, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_4",  "[Channel2]", "hotcue_2",    false, KontrolF1.hotcueCallback, [0x7f, 0x00, 0x7f]);
    KontrolF1.linkGrid("decks", "grid_5",  "[Channel1]", "hotcue_3",    false, KontrolF1.hotcueCallback, [0x00, 0x40, 0x7f]);
    KontrolF1.linkGrid("decks", "grid_6",  "[Channel1]", "hotcue_4",    false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_7",  "[Channel2]", "hotcue_3",    false, KontrolF1.hotcueCallback, [0x00, 0x40, 0x7f]);
    KontrolF1.linkGrid("decks", "grid_8",  "[Channel2]", "hotcue_4",    false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_9",  "[Channel1]", "loop_in",     false, undefined,        [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_10", "[Channel1]", "loop_out",    false, undefined,        [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_11", "[Channel2]", "loop_in",     false, undefined,        [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_12", "[Channel2]", "loop_out",    false, undefined,        [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("decks", "grid_13", "[Channel1]", "quantize",    true,  undefined,        [0x10, 0x10, 0x40]);
    KontrolF1.linkGrid("decks", "grid_14", "[Channel1]", "reloop_exit", false, undefined,        [0x7f, 0x00, 0x00]);
    ...
    KontrolF1.linkGrid("samplers", "grid_1",  "[Sampler1]", "hotcue_1", false, KontrolF1.hotcueCallback, [0x7f, 0x00, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_2",  "[Sampler2]", "hotcue_1", false, KontrolF1.hotcueCallback, [0x7f, 0x00, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_3",  "[Sampler3]", "hotcue_1", false, KontrolF1.hotcueCallback, [0x7f, 0x00, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_4",  "[Sampler4]", "hotcue_1", false, KontrolF1.hotcueCallback, [0x7f, 0x00, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_5",  "[Sampler1]", "hotcue_2", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_6",  "[Sampler2]", "hotcue_2", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_7",  "[Sampler3]", "hotcue_2", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_8",  "[Sampler4]", "hotcue_2", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x00]);
    KontrolF1.linkGrid("samplers", "grid_9",  "[Sampler1]", "hotcue_3", false, KontrolF1.hotcueCallback, [0x00, 0x00, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_10", "[Sampler2]", "hotcue_3", false, KontrolF1.hotcueCallback, [0x00, 0x00, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_11", "[Sampler3]", "hotcue_3", false, KontrolF1.hotcueCallback, [0x00, 0x00, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_12", "[Sampler4]", "hotcue_3", false, KontrolF1.hotcueCallback, [0x00, 0x00, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_13", "[Sampler1]", "hotcue_4", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_14", "[Sampler2]", "hotcue_4", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_15", "[Sampler3]", "hotcue_4", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x7f]);
    KontrolF1.linkGrid("samplers", "grid_16", "[Sampler4]", "hotcue_4", false, KontrolF1.hotcueCallback, [0x00, 0x7f, 0x7f]);
...
KontrolF1.setButtonBrightness = function(name, value) {
    var controller = KontrolF1.controller;
...
// Set specified value for 7 segment display
KontrolF1.set7SegmentNumber = function(number) {
    var controller = KontrolF1.controller;
    var packet = controller.getOutputPacket("lights");
...
KontrolF1.set7SegmentValue = function(name, dp, v1, v2, v3, v4, v5, v6, v7) {
    var controller = KontrolF1.controller;
    var packet = controller.getOutputPacket("lights");
    var field = undefined;
...
KontrolF1.setPADColor = function(index, red, green, blue) {
    var controller = KontrolF1.controller;
    var packet = controller.getOutputPacket("lights");
    var field = undefined;

    if (index <= 0 || index > 16) {
        HIDDebug("Invalid grid LED index: " + index);
        return;
    }
...
KontrolF1.disconnectModeLEDs = function(mode) {
    ...
    for (var name in grid) {
        ...
        if (button.ledname == undefined)
            continue
        engine.connectControl(button.group, button.ledname, KontrolF1.setLEDCallback, false);
    }
...
KontrolF1.connectModeLEDs = function(mode) {
    ...
    for (var name in grid) {
        ...
        if (button.ledname == undefined)
            continue;
        engine.connectControl(button.group, button.ledname, KontrolF1.setLEDCallback);
...
KontrolF1.setLEDCallback = function(value, group, key) {
...
KontrolF1.knobCallback = function(field) {
    var controller = KontrolF1.controller;
    ...
    return KontrolF1.controlCallback(knob,field);
}

KontrolF1.faderCallback = function(field) {
    var controller = KontrolF1.controller;
    ...
    return KontrolF1.controlCallback(fader,field);
}

KontrolF1.gridCallback = function(field) {
    ...
    return KontrolF1.buttonCallback(button,field);
}

KontrolF1.playCallback = function(field) {
...
KontrolF1.controlCallback = function(control, field) {
...
KontrolF1.buttonCallback = function(button, field) {
    var controller = KontrolF1.controller;
...
KontrolF1.hotcueCallback = function(button, field) {
    var controller = KontrolF1.controller;
    var name;
